...
// this variable specify the debug type (0 => nothing, 1 => sql result, 2 => loop checking, 3 other value checking)
$FG_DEBUG = 0;

// Define the color of the head table
...
// Define the color of the admin row
$FG_TABLE_ROW_COLOR_ADMIN = "#FCCDCA";

// Var_col would define the col that we want show in your table
...
// LIMITE_DISPLAY define the limit of record to display by page
...
// $FG_EDITION define if you want process to the edition of the database record
$FG_VOICE_RIGHT = true;
$FG_KICKOUT = true;

// This will store the total number of column
$FG_TOTAL_TABLE_COL = $FG_NB_TABLE_COL;
if ($FG_VOICE_RIGHT || $FG_KICKOUT) $FG_TOTAL_TABLE_COL++;

// This define the width of the HTML table
...
if ($_SESSION['auth'] && $invite_num != "" && $data != "" && $bookid != "") {
    if ($action == 'outboundcall' || $action == 'quickcall') {
        ...
        $res = $as->connect();
        if (!$res) {
            echo _("Error connection to the manager") . "!";
            exit();
        }
        ...
        //echo $callerid;
        $res = $as->Originate($channel, $exten, $context, $priority, $timeout, $callerid, $variable, $account, $application, $data, null);
...
<body bgColor=#acbdee>
    ...
    <center>
        <?php if ($action == "quickcall") {
            ...
            <input type="submit" value="Back for next invite" onclick="javascript:window.history.back();">
            <br>
            <input type="submit" value="Close a window" onClick="window.close()" />
        <?php } else { ?>
                        ...
                        <TBODY>
                            ...
                            <TR>
                                <td width="165" valign="top" bgcolor="#D3D3F2">
                                    <table width="100%" border="0" cellpadding="2" cellspacing="0">
                                        <tr>
                                            <td><b><?php echo _("APPLICATION"); ?></b> </td>
                                        </tr>
                                    </table>
                                </td>
                                <TD width="405" valign="top">
