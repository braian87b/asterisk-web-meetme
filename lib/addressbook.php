...
?>
    <center>
        ...
        if (!$error_flag) {
            if (isset($search)) {
                $FG_CLAUSE = "`listOwner`='$FG_USER' AND (`name` like '%$search%' OR `surname` like '%$search%' OR `number` like '%$search%' OR `email` like '%$search%') ";
            }

            $FG_CLAUSE .= " ORDER BY `name` ASC ";

            if (isset($of))
                $FG_CLAUSE .= "LIMIT $of, $col";
        ...
        ?>
            <form method="POST" action="<?php echo preg_replace("/&of=\d+|&deladdres=\d+/", "", $_SERVER['REQUEST_URI']) . "&listupdate=1"; ?>">
                <table border="1" cellpadding="5" cellspacing="0" align="center" width="800">
                    ...
                    <table border="1" cellpadding="5" cellspacing="0" align="center" width="800">
                        <tr>
                            <td align="center" colspan="<?php echo $NumOfLists + 1; ?>"><?php echo _("Personal lists"); ?>
                            </td>
                        </tr>
                        <tr>
