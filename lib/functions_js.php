<?php include("locale.php"); ?> <script language="JavaScript" type="text/JavaScript">
// functions.js
var browserType;	
...
function conf_rxtxcurent(channel, command) {
  window.open ('conf_rxtxcurent.php?channel='+channel+'&command='+command, 'newWin', 'toolbar=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=50,height=50')
}
...
function addRowToTable(tableName)
{
  ...
  if(lastRow == 0)
  {
    ...
    oCell.innerHTML =<?php echo "'" . _("First name") . "'"; ?>;

    oCell = row.insertCell(1);
    oCell.align = "center";
    oCell.style.fontWeight = "bold";
    oCell.innerHTML =<?php echo "'" . _("Last name") . "'"; ?>;

    oCell = row.insertCell(2);
    oCell.align = "center";
    oCell.style.fontWeight = "bold";
    oCell.innerHTML =<?php echo "'" . _("Email") . "'"; ?>;

    oCell = row.insertCell(3);
    oCell.align = "center";
    oCell.style.fontWeight = "bold";
    oCell.innerHTML =<?php echo "'" . _("Telephone") . "'"; ?>;

    oCell = row.insertCell(4);
    oCell.align = "center";
    oCell.innerHTML =<?php echo "'" . _("Delete") . "'"; ?>;
    oCell.style.fontWeight = "bold";
  ...
  el.type = 'button';
  el.name = 'remove';
  el.setAttribute('value', <?php echo "'" . _("Delete") . "'"; ?>);
  if(msie > 0)
  {
...
function vemail(obj)
{
  ...
  if (goodEmail){
    ...
    alert(<?php echo "\"" . _("Please enter a valid email address") . ".\""; ?>)
...
function call_add(name, invite_num, rx, tx, data, bookid) {
  var req = getXmlHttp();
  var statusElem = document.getElementById(invite_num);
  ...
  req.open('GET', 'call_operator_add.php?name=' + name +
                  '&invite_num=' + invite_num +
                  '&action=quickcall&data=' + data +
                  '&bookid=' + bookid +
                  '&rx=' + rx +
                  '&tx=' + tx,
                  true);
