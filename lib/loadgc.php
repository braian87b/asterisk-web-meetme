...
$mtime = @filemtime ('/tmp/googleCalSync');
$now = time ();
if ($mtime && $now - $mtime < 5 * 60)
  exit;

fclose (fopen ('/tmp/googleCalSync', 'w'));
...
/* This loop resets the "US Conference System" to be marked as not
   reserved by an event so that it can be assigned to another conference
   at the same time.  We do this as a separate loop to make sure it's done
   as quickly as possible.  */
foreach ($events['items'] as $evt)
  if (!isset ($evt['transparency']) || $evt['transparency'] != 'transparent')
    patch_gcal_event (GCAL_RESOURCE, $evt['id'],
